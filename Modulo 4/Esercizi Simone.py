"""
#Esercizio 1: Stringa al contrario
...
stringa = str(input("Inserisci la stringa da elaborare: "))
x = 1
while x <= len(stringa):
    print(stringa[-x], end= "")
    x += 1
...
while True:
    try:
        lista.append(int(input("Inserisci un elemento della lista od una lettera per uscire: ")))
    except:break
x = 0
while x < len(lista):
    if lista[x]%2 == 0:
...
while True:
    p_word = str(input("Inserire la password.\nRicorda che deve essere minimo di 8 caratteri, con un carattere speciale fra (-,!,£,$,%,&,|), una maiuscola, una minuscola e due numeri\n"))
    count = 0
    ctrl1 = False
    ctrl2 = False
    ctrl3 = False
    if len(p_word)>=8:
        for el in p_word:
            if el in special: ctrl1 = True
            if el.isupper():ctrl2 = True
            if el.isnumeric():count += 1
            if el.islower():ctrl3 = True 
    if ctrl1 == True and ctrl2 == True and ctrl3 == True and count >=2: 
        print("La password va bene.")
        break
    else:print("La password non va bene, riprovare!\nSeguire gli/l' alert")
    if len(p_word)<8: print("La password è troppo corta.")
    if ctrl1 == False: print("Manca il carattere speciale.")
    if ctrl2 == False: print("Manca il carattere maiuscolo.")
    if ctrl3 == False: print("Manca il carattere minuscolo.")
    if count < 2: print("Non ci sono abbastanza numeri.")        



#Esercizio 4: Verifica dei Palindromi
#Scrivi un programma Python che chiede all'utente di inserire una stringa e verifica se è un palindromo (una parola, frase, numero o altra sequenza di caratteri che si legge allo stesso modo in avanti e all'indietro).
#Utilizza un ciclo while e istruzioni if per effettuare il controllo e fornire un feedback appropriato.

pal = str(input("Inserire la parola da esaminare:\n"))
ex = str("")
x = 1
while x <= len(pal):
    ex += pal[-x]
    x += 1
if pal == ex:print("Il testo inserito è palindromo")
else: print("Il testo non è palindromo")



#Esercizio 5: Gioco dell'impiccato
#Crea un gioco in cui il programma seleziona una parola casuale da una lista predefinita e l'utente deve indovinare la parola lettera per lettera. 
#Utilizza un ciclo while e istruzioni if per guidare l'utente e fornire feedback. Includi un limite al numero di tentativi errati consentiti e dai un suggerimento se l'utente si blocca.
"""
import random
err = False
...
while True:                                                                                                                 #Scelta della lunghezza della parola
    print("Scegli di quanti caratteri è la parola con cui giocare.\nRicorda che la scelta è fra 3,4 o 5 caratteri.")
    try:    
...
while True:                                                                                                                 #Scelta della lettera
    err = False
    x=0
    while True:
    ...
    if ("".join(player)) == word:
        print("Complimenti, hai trovato la parola e salvato il nostro stickman =)")
        break
